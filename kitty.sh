...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/bobhggfgg/kitty-custom-20260502/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/bobhggfgg/kitty-custom-20260502/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/kitty -N --no-check-certificate https://raw.githubusercontent.com/bobhggfgg/kitty-custom-20260502/main/kitty.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/bobhggfgg/kitty-custom-20260502 ---
  ${green}0.${plain} 修改配置
————————————————
